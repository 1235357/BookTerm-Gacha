"""
BookTerm Gacha - Text Helper Module
====================================

This module provides comprehensive text analysis utilities for multi-language
character detection and classification. It's essential for NER and LLM
processing of books in various languages.

Supported Languages:
    - CJK: Chinese, Japanese, Korean (Hanzi/Kanji/Hanja)
    - JA: Japanese (Hiragana, Katakana)
    - KO: Korean (Hangul)
    - Latin: English and European languages
    - RU: Russian (Cyrillic)
    - DE/FR/ES/IT/PT: European languages with diacritics
    - TH/ID/VI: Southeast Asian languages

Key Functions:
    - is_punctuation(): Detect punctuation across all languages
    - any_*(): Check if text contains specific character types
    - get_display_length(): Calculate visual display width

Character Sets:
    Uses precise Unicode ranges to distinguish between:
...
Usage:
    ...
    if TextHelper.JA.any_hiragana(text):
        print("Contains hiragana")
