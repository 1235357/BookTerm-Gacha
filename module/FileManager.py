"""
BookTerm Gacha - File Manager Module
=====================================

This module handles all file I/O operations for reading source texts and
writing results. It supports multiple file formats commonly used in
book translation workflows.

Supported Input Formats:
    - EPUB: Electronic publication format (most common for books)
    - TXT: Plain text files
    - MD: Markdown files
    - ASS/SRT: Subtitle files
    - XLSX: Excel spreadsheets
    - TRANS: Translation project files
    - RENPY: Ren'Py visual novel scripts
    - Various JSON formats (KVJSON, MESSAGEJSON)

Key Features:
    - Automatic format detection based on file extension
    - Traditional/Simplified Chinese conversion (OpenCC)
    - Batch processing of input folders
    - Normalized text output for consistency
    - Character/name mapping support

Output Formats:
    - JSON: Machine-readable term list with metadata
    - MD: Human-readable Markdown summary
    - XLSX: Excel format for editing

Dependencies:
    - opencc: Chinese character conversion
    - ebooklib: EPUB parsing (via EPUB submodule)
...
class FileManager():
    ...
    # OpenCC 简繁转换器（类级别，延迟加载）
    _opencc_s2t: opencc.OpenCC = None
    _opencc_t2s: opencc.OpenCC = None
    ...
    def convert_chinese(self, text: str) -> str:
        """根据配置转换中文字形"""
        if self.traditional_chinese_enable and text:
            return self.get_opencc_s2t().convert(text)
        return text
        ...
        with open(path, "w", encoding = "utf-8") as writer:
            for k, word in enumerate(words):
                ...
                if getattr(word, "surface_translation", "") != "":
                    writer.write(f"词语翻译 : {self.convert_chinese(word.surface_translation)}" + "\n")

                if getattr(word, "gender", "") != "":
                    writer.write(f"角色性别 : {self.convert_chinese(word.gender)}" + "\n")

                if getattr(word, "context_summary", "") != "":
                    writer.write(f"语义分析 : {self.convert_chinese(word.context_summary)}" + "\n")
                ...
                if len(getattr(word, "context_translation", [])) > 0:
                    writer.write("参考文本翻译 : ※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※" + "\n")
                    translated = FileManager.RE_DUPLICATE.sub("\n", "\n".join(word.context_translation))
                    writer.write(f"{self.convert_chinese(translated)}" + "\n")
    ...
    def write_dict_to_file(self, words: list[Word], path: str, language: int) -> None:
        with open(path, "w", encoding = "utf-8") as file:
            data = {}
            for k, word in enumerate(words):
                data[word.surface] = self.convert_chinese(word.surface_translation)
    ...
    def write_glossary_to_file(self, words: list[Word], path: str, language: int) -> None:
        with open(path, "w", encoding = "utf-8") as file:
            ...
            for word in words:
                ...
                data["dst"] = self.convert_chinese(word.surface_translation)

                if word.group == "角色" and "男" in word.gender:
                    data["info"] = self.convert_chinese("男性名字")
                elif word.group == "角色" and "女" in word.gender:
                    data["info"] = self.convert_chinese("女性名字")
                elif word.group == "角色":
                    data["info"] = self.convert_chinese("名字")
                else:
                    data["info"] = self.convert_chinese(f"{word.group}")
    ...
    def write_galtransl_to_file(self, words: list[Word], path: str, language: int) -> None:
        with open(path, "w", encoding = "utf-8") as file:
            for word in words:
                line = f"{word.surface}\t{self.convert_chinese(word.surface_translation)}"

                if word.group == "角色" and "男" in word.gender:
                    line = line + f"\t{self.convert_chinese('男性名字')}"
                elif word.group == "角色" and "女" in word.gender:
                    line = line + f"\t{self.convert_chinese('女性名字')}"
                elif word.group == "角色":
                    line = line + f"\t{self.convert_chinese('名字')}"
                else:
                    line = line + f"\t{self.convert_chinese(word.group)}"
