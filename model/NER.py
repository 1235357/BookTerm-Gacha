"""
BookTerm Gacha - NER (Named Entity Recognition) Module
=======================================================

This module implements BERT-based Named Entity Recognition for extracting
character names, locations, and other named entities from text. It supports
multiple languages including Chinese, Japanese, Korean, and English.

Key Features:
    - BERT NER model with bf16 optimization for GPU acceleration
    - Multi-language tokenization (PeCab for Korean, Sudachi for Japanese)
    - Blacklist filtering to exclude common/unwanted terms
    - Fake name generation for privacy-sensitive testing
    - Batch processing with configurable batch sizes

Entity Types:
    - PER: Person names (characters)
    - LOC: Locations (places, buildings)
    - ORG: Organizations (optional, filtered by default)
    - PRD: Products/Works (optional, filtered by default)

Model Path:
    resource/kg_ner_bf16 (BERT model fine-tuned for multi-language NER)

Dependencies:
    - transformers: Hugging Face transformers for BERT
    - torch: PyTorch for GPU acceleration
    - pecab: Korean morphological analyzer
...
class NER:
    ...
    def __init__(self) -> None:
        ...
        # NER 目标实体类型（默认只保留人名和地点）
        self.target_types = {"PER", "LOC"}
    ...
    # 设置 NER 目标实体类型
    def set_target_types(self, types: list[str]) -> None:
        """
        设置 NER 阶段保留的实体类型
        可选值：PER(人名), LOC(地点), ORG(组织), PRD(产品/作品)
        """
        self.target_types = set(types)
        LogHelper.debug(f"NER 目标类型已设置为: {self.target_types}")
    ...
    def verify_by_language(self, text: str, language: int) -> bool:
        ...
        if language == NER.Language.JA:
            ...
            # 【关键】只保留包含假名的实体！
            # 术语表是针对假名做的，纯汉字的日文词条不需要（如"田中"、"東京"）
            # 因为这些在翻译时可以直接保留或简单转换
   ...
   # 代码转换为姓名
    ...
    def search_for_entity(self, input_lines: list[str], names: dict, nicknames: dict, language: int) -> tuple[list, dict]:
        ...
        with LogHelper.status("正在对文本进行预处理 ..."):
            seen = set()
            fake_name_mapping = {}
            ruby_cleaned_count = 0
            for i, line in enumerate(input_lines):
                # 清理注音标记（日文振り仮名/ルビ）
                if language == NER.Language.JA:
                    original_line = line
                    line = RubyCleaner.clean(line)
                    if line != original_line:
                        ruby_cleaned_count += 1
        ...
        with ProgressHelper.get_progress() as progress:
            ...
            for result in self.classifier(self.generator(chunks), batch_size = self.bacth_size):
                ...
                for token in result:
                    ...
                    # 【NER 阶段粗筛】根据配置的目标类型过滤
                    # 默认只保留人名(PER)和地点(LOC)，过滤组织(ORG)和产品(PRD)
                    # 这样可以大幅减少后续"参考文本翻译"和"词义分析"的 LLM 调用次数
                    if entity_group not in self.target_types:
                        continue
        ...
        if ruby_cleaned_count > 0:
            LogHelper.info(f"[查找实体词语] 已清理 [green]{ruby_cleaned_count}[/] 行注音标记（振り仮名/ルビ）...")
        if len(seen) > 0:
